...
\begin{document}
	...
	\begin{frame}
		...
		\begin{abstract}
			In classical signal analysis, the autocorrelation function (ACF) is used to estimate the signal power and the signal-to-noise ratio. Assuming, that signal and noise are uncorrelated stochastic processes, the signal power can be obtained by subtracting the noise power from the power of the signal in noise. This demands auxiliary signal data exclusively consisting of noise.
			
			However, there are situations in which signal areas that only contain noise are difficult to identify or only little data is available. In such cases, considerable losses in accuracy may have to be accepted when estimating the signal power.
			
			The method introduced here, promises to significantly reduce the influence of noise in ACF estimation without the need for auxiliary noise data. The technique involves replacing the ACF magnitudes in the area around the zero-lag with a regression polynomial. In it's initial design, the method is applicable to damped, sinusoidal signals.
	...
	\begin{frame}
		...
		\begin{columns}
			...
			\end{RIPcolleft}
			\begin{RIPcolright}
				\textbf{Auto-correlation function of a sine}
				\begin{itemize}
	...
	\begin{frame}[allowframebreaks]
		\frametitle{\appendixname{} \textemdash{} Algorithm ACF-RN}\label{app:algorithm}
		...
		\begin{algorithmic}[1]
			...
			\State $c_P \gets
				\underset{c_0,c_1,c_2}{\arg}\min( \frac{1}{L_{reg}} \sum\limits_{m \in I_{reg}} (R_{xx,1}[m] - P[m,c_P))^2 )$
